...
class Sheet:
    ...
    def __init__(self, credentials: str, sheet_name: str, starting_column: str, ending_column: str) -> None:
        self.__active_sheet_page = gspread.service_account(
            credentials).open(sheet_name).sheet1
    ...
    def get_state(credentials: dict, sheet_name: str, starting_column: str, ending_column: str):
        if not Sheet.instance:
            Sheet.instance = Sheet(
                credentials, sheet_name, starting_column, ending_column)
        return Sheet.instance

    def __fetch(self) -> None:
        try:
            self.__cached_data = self.__active_sheet_page.get_all_records()
    ...
    def __find(self, problem_number: str):
        cell = self.__active_sheet_page.find(problem_number)
    ...
    def get_entry(self, problem_number: int):
        ...
        return [row_number, self.__active_sheet_page.row_values(cell.row)]

    def create_entry(self, entries: list) -> None:
        try:
            self.__active_sheet_page.insert_row(entries, 2)
            self.__active_sheet_page.sort((1, 'asc'))
        except Exception as error:
    ...
    def update_entry(self, row_number: int, updated_data: list) -> None:
        ...
        try:
            self.__active_sheet_page.update(cell_range, updated_data)
        except Exception as error:
            print(error)
    ...
    def delete_entry(self, problem_number: int) -> None:
        ...
        try:
            self.__active_sheet_page.delete_rows(int(cell.row))
        except Exception as error:
            print(error)
    ...
    def delete_all_entries(self) -> None:
        try:
            self.__active_sheet_page.clear()
        except Exception as error:
            print(error)
